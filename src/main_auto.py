...
class NotificadorDiscord:
    ...
    async def reportar_estrategia(self, roles):
        ...
        embed = {
            "title": "🧠 Sistema Híbrido Iniciado",
    ...
    async def reportar_exito(self, titulo, url, qa_log, portal_nombre):
        ...
        texto_qa = "\n\n".join(qa_log) if qa_log else "🚀 Postulación Express: El empleador solo pidió el CV, no había cajas de texto para rellenar."
        ...
        embed = {
            "title": f"✅ ¡Éxito en {portal_nombre}!",
...
class OrquestadorBot:
    ...
    async def iniciar(self):
        print("🧠 Consultando IA (Arquitectura Híbrida)...")
        ...
        roles_limpios = []
        for r in roles:
            stats_rol = self.memoria.data.get(r, {})
            if stats_rol.get("exitos", 0) > 0 or stats_rol.get("descartes", 0) < 30:
                roles_limpios.append(r)
            elif random.random() < 0.15:
                print(f"   🎲 Explorando categoría bloqueada: {r}")
                roles_limpios.append(r)
        ...
        async with async_playwright() as p:
            ...
            for cargo in roles_limpios:
                ...
                res = await asyncio.gather(
                    self.ct.obtener_enlaces(p1, termino_amplio, cantidad=60),
                    self.lab.obtener_enlaces(p2, termino_amplio, cantidad=60),
                    self.tr.obtener_enlaces(p3, termino_amplio, cantidad=60)
                )
                ...
                for oferta in enlaces:
                    ...
                    # 👇 SÚPER-ESCUDO PARA AHORRAR TOKENS Y TIEMPO
                    palabras_prohibidas = [
                        "full time", "fulltime", "full-time", "44 horas", "45 horas", "40 hrs",
                        "guardia", "cocina", "actualizado hace", "publicado hace",
                        "nuevo", "destacada", "práctica", "practica", "supervisor", "jefe"
                    ]
                    if any(bad in oferta['titulo'].lower() for bad in palabras_prohibidas):
                        print(f"   ⚡ Filtro rápido (Descartado al instante: {oferta['titulo']}).")
                    ...
                    if evaluacion.get("puntaje", 0) >= 65:
                        ...
                        print("   ❌ Puntaje IA bajo.")
                        self.memoria.registrar(cargo, False)
                        self.ct.registrar_url(oferta['url'])
                await p_oferta.close()
            await browser.close()

        await self.discord.notificar_estado(f"Apagado normal. Meta alcanzada o mercado escaneado por completo. Postulaciones hoy: {self.exitos}")

# 👇 BLOQUE FINAL REPARADO: Ya no ocultará los errores
if __name__ == "__main__":
    bot = OrquestadorBot()
    try:
        asyncio.run(bot.iniciar())
    except KeyboardInterrupt:
        print("\n\n🛑 APAGADO SEGURO ACTIVADO.")
        asyncio.run(bot.discord.notificar_estado(f"Apagado forzado por el usuario (Ctrl+C). Postulaciones logradas: {bot.exitos}"))
    except Exception as e:
        print(f"\n❌ ERROR FATAL INESPERADO: {e}")
    finally:
        bot.memoria.guardar()
        print(f"📊 Tareas exitosas: {bot.exitos}")